...
VPS_SERVICE_NAME = os.getenv('VPS_SERVICE_NAME', 'CloudNode')
WATERMARK = f"{VPS_SERVICE_NAME} VPS Service"
WELCOME_MESSAGE = os.getenv('VPS_WELCOME_MESSAGE', f'Welcome To {VPS_SERVICE_NAME}! Get Started With Us!')
...
RUN echo '{welcome_message}' > /etc/motd && \\
    ...
    echo '{service_name_lower}-{vps_id}' > /etc/hostname
...
async def build_custom_image(vps_id, username, root_password, user_password, base_image=DEFAULT_OS_IMAGE):
    ...
    try:
        ...
        dockerfile_content = DOCKERFILE_TEMPLATE.format(
            ...
            service_name_lower=VPS_SERVICE_NAME.lower(),
            vps_id=vps_id
        )
        ...
        # Build the image
        image_tag = f"{VPS_SERVICE_NAME.lower()}/{vps_id.lower()}:latest"
...
async def setup_container(container_id, status_msg, memory, username, vps_id=None, use_custom_image=False):
    ...
    try:
        ...
        hostname_cmd = f"echo '{VPS_SERVICE_NAME.lower()}-{vps_id}' > /etc/hostname && hostname {VPS_SERVICE_NAME.lower()}-{vps_id}"
...
async def on_ready():
    ...
    try:
        await bot.change_presence(activity=discord.Activity(type=discord.ActivityType.watching, name=f"{VPS_SERVICE_NAME} VPS"))
...
async def create_vps(ctx, memory: int, cpu: int, disk: int, user: discord.User, os_image: str = DEFAULT_OS_IMAGE, use_custom_image: bool = True):
    ...
    try:
        ...
        container_config = {
            ...
            'hostname': f'{VPS_SERVICE_NAME.lower()}-{vps_id}',
        ...
        try:
            embed = discord.Embed(
                title=f"🚀 Your {VPS_SERVICE_NAME} VPS is Ready!",
            ...
            embed.set_footer(text=f"{VPS_SERVICE_NAME} VPS Service")
            
            await user.send(embed=embed)
        except discord.Forbidden:
            await ctx.send(f"❌ Could not send DM to {user.mention}. Please enable DMs from server members.")
        
        await status_msg.edit(content=f"✅ {VPS_SERVICE_NAME} VPS created successfully for {user.mention}! Check your DMs for details.")
...
async def connect_vps(ctx, token: str):
    ...
    try:
        ...
        embed = discord.Embed(
            title=f"🔗 Connect to {VPS_SERVICE_NAME} VPS `{vps_data['vps_id']}`",
...
async def list_vps(ctx):
    ...
    if not vps_list:
        embed = discord.Embed(
            title=f"📋 Your {VPS_SERVICE_NAME} VPS Instances",
...
async def admin_vps_list(ctx):
    ...
    embed = discord.Embed(
        title=f"📋 All {VPS_SERVICE_NAME} VPS Instances",
...
async def admin_stats(ctx):
    ...
    try:
        ...
        embed = discord.Embed(
            title=f"📊 {VPS_SERVICE_NAME} System Statistics",
            color=discord.Color.green(),
            timestamp=datetime.datetime.now()
        )
